...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        scores = rcnn_util.batch_slice([scores, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        deltas = rcnn_util.batch_slice([deltas, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        anchors = rcnn_util.batch_slice(ix, lambda x: tf.gather(anchors, x),
                                    self.config.IMAGES_PER_GPU,
                                    names=["pre_nms_anchors"])

        # Apply deltas to anchors to get refined anchors.
        # [batch, N, (y1, x1, y2, x2)]
        boxes = rcnn_util.batch_slice([anchors, deltas],
        ...
        boxes = rcnn_util.batch_slice(boxes,
        ...
        def nms(normalized_boxes, scores):
            ...
            return proposals
        proposals = rcnn_util.batch_slice([normalized_boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        return proposals
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    deltas = rcnn_util.box_refinement_graph(positive_rois, roi_gt_boxes)
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        detections_batch = rcnn_util.batch_slice(
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    image, window, scale, padding = rcnn_util.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        mode=config.IMAGE_RESIZE_MODE)
    mask = rcnn_util.resize_mask(mask, scale, padding)
    ...
    bbox = rcnn_util.extract_bboxes(mask)
    ...
    if use_mini_mask:
        mask = rcnn_util.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    for i in range(overlaps.shape[1]):
        gt = gt_boxes[i]
        overlaps[:, i] = rcnn_util.compute_iou(
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    bboxes[pos_ids, roi_gt_class_ids[pos_ids]] = rcnn_util.box_refinement(
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        # Compute overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = rcnn_util.compute_overlaps(anchors, crowd_boxes)
    ...
    overlaps = rcnn_util.compute_overlaps(anchors, gt_boxes)
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    anchors = rcnn_util.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Generate Anchors
        self.anchors = rcnn_util.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                                                      config.RPN_ANCHOR_RATIOS,
                                                      config.BACKBONE_SHAPES,
                                                      config.BACKBONE_STRIDES,
                                                      config.RPN_ANCHOR_STRIDE)
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = rcnn_util.resize_image(
                image,
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        window = rcnn_util.norm_boxes(window, image_shape[:2])
        ...
        boxes = rcnn_util.denorm_boxes(boxes, original_image_shape[:2])
        ...
        for i in range(N):
            ...
            full_mask = rcnn_util.unmold_mask(masks[i], boxes[i], original_image_shape)
